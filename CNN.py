...
optimizer_base = keras.optimizers.RMSprop(learning_rate=0.0001)
model_base.compile(optimizer=optimizer_base, loss="categorical_crossentropy")
...
# Train models
hist_nt = model_base.fit(X_subset.reshape(-1, 28, 28, 1), y_nt_subset, validation_split=0.33, epochs=10, batch_size=32, verbose=0)
hist_asd = model_base.fit(X_subset.reshape(-1, 28, 28, 1), y_asd_subset, validation_split=0.33, epochs=10, batch_size=32, verbose=0)
...
accuracies = accuracies.melt(ignore_index=False)
...
weights = pd.DataFrame(np.column_stack([w_nt, w_asd, w_syn, w_hom]), columns=['NT', 'ASD', 'SYN', 'HOM'])

# Plot weights in a violin plot
