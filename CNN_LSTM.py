...
from keras.layers import Reshape, Dense, Conv2D, MaxPooling2D, Flatten, LSTM
...
# Neurotypical CNN-LSTM model
lstm_nt = Sequential([
  Conv2D(32, kernel_size=(3, 3), activation='relu', input_shape=(28, 28, 1)),
  MaxPooling2D(pool_size=(2, 2)),
  Reshape((13, 13*32)),
  LSTM(64, activation='sigmoid', return_sequences=False, name='CA3'),
  Dense(96, activation='relu'),
  Dense(10, activation='softmax')])
optimizer_nt = keras.optimizers.RMSprop(learning_rate=0.01)
lstm_nt.compile(optimizer=optimizer_nt, loss='categorical_crossentropy', metrics=['accuracy'])
...
lstm_asd = Sequential([
  ...
  Dense(96, activation='relu'),
  Dense(10, activation='softmax')])
optimizer_asd = keras.optimizers.RMSprop(learning_rate=0.01)
lstm_asd.compile(optimizer=optimizer_asd, loss='categorical_crossentropy', metrics=['accuracy'])

# Synaptic plasticity CNN-LSTM model
lstm_syn = Sequential([
  Conv2D(32, kernel_size=(3, 3), activation='relu', input_shape=(28, 28, 1)),
  MaxPooling2D(pool_size=(2, 2)),
  Reshape((13, 13*32)),
  ...
  Dense(96, activation='relu'),
  Dense(10, activation='softmax')])
optimizer_syn = keras.optimizers.RMSprop(learning_rate=0.005)
lstm_syn.compile(optimizer=optimizer_syn, loss='categorical_crossentropy', metrics=['accuracy'])
...
lstm_hom = Sequential([
  ...
  Dense(96, activation='relu'),
  Dense(10, activation='softmax')])
optimizer_hom = keras.optimizers.RMSprop(learning_rate=0.01, weight_decay=-0.5)
lstm_hom.compile(optimizer=optimizer_hom, loss='categorical_crossentropy', metrics=['accuracy'])

# Train models
hist_nt = lstm_nt.fit(X_subset.reshape(-1, 28, 28, 1), y_nt_subset, validation_split=0.33, epochs=10, batch_size=128, verbose=0)
hist_asd = lstm_asd.fit(X_subset.reshape(-1, 28, 28, 1), y_asd_subset, validation_split=0.33, epochs=10, batch_size=128, verbose=0)
hist_syn = lstm_syn.fit(X_subset.reshape(-1, 28, 28, 1), y_nt_subset, validation_split=0.33, epochs=10, batch_size=128, verbose=0)
hist_hom = lstm_hom.fit(X_subset.reshape(-1, 28, 28, 1), y_nt_subset, validation_split=0.33, epochs=10, batch_size=128, verbose=0)
...
w_nt = np.mean(lstm_nt.get_layer('CA3').get_weights()[0], axis=1)
w_asd = np.mean(lstm_asd.get_layer('CA3').get_weights()[0], axis=1)
w_syn = np.mean(lstm_syn.get_layer('CA3').get_weights()[0], axis=1)
w_hom = np.mean(lstm_hom.get_layer('CA3').get_weights()[0], axis=1)
